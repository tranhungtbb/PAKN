...
export class UpdateGroupUserComponent implements OnInit {
  ...
  ngOnInit() {
    this.updateGroupUserForm = this.formBuilder.group({
      ...
      maNhom: ['', [Validators.required, Validators.pattern('[0-9a-zA-Z_-]+')]],
      moTa: ['', []],
      nguoiTao: ['', []],
      trangThai: ['', []],
      xoa: ['', []] 
  ...
  validateModel() {
    ...
    if (this.model.maNhom == "" || this.model.tenNhom == "") {
      return false;
    }
    return true;
  }
  ...
  onSave() {
    this.submitted = true;
    if (this.updateGroupUserForm.invalid) {
      return;
    }
    ...
    this.groupUserservice.UpdateGroupUser(request).subscribe(data => {
      if (data.status === 1) {
        this.model = data.GroupUser;
        this.toastr.success('Cập nhật thành công');
        this.router.navigate(["/business/system-management/group-user-management"]);
      } else {

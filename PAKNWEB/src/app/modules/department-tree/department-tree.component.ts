...
import { MenuPassingObject } from '../../modules/system-management/components/orgnization/menuPassingObject';
import { DepartmentMenuType } from '../../modules/system-management/components/orgnization/departmentMenuType';
// declare var jquery: any;
declare var $: any;

@Component({
  selector: 'department-treeprime',
...
export class DepartmentTreeComponent implements OnInit {
  ...
  getDepartmentTree(): void {
    this.service.getTreeUnit()
      .subscribe(data => {
        if (data.unitTree.length > 0) {
          data.unitTree[0].maCapCha = null;
          this.depid = data.unitTree[0].ma;

          this.object.type = DepartmentMenuType.DanhSachNhomNguoiDung;
          this.object.data = data.unitTree[0];
          this.onMainDepartmentMenuClick.emit(this.object);

          this.newtrenode = this.getDataByParentId(data.unitTree, null);

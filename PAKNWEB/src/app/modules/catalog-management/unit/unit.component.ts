...
import { UnitObject } from '../../../models/unitObject';
...
export class UnitComponent implements OnInit {
  ...
  listUnit = new Array<UnitObject>();
  listTrangThai: any = [
    { value: true, text: "Hiệu lực" },
    { value: false, text: "Hết hiệu lực" }
  ];
  form: FormGroup;
  model: any = new UnitObject();
  ...
  getList() {
    ...
    this._service.UnitGetList(request).subscribe(response => {
      if (response.status == 1) {
        this.listUnit = [];
        this.listUnit = response.listUnit;
  ...
  exportExcel() {
    ...
    this._service.ExportExcel(request).subscribe(
      response => {
        ...
        var fileName = "DM_DonViTrucThuocKTNN_" + yyyy + mm + dd+hh+minute;

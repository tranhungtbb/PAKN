...
namespace PAKNAPI.ModelBase
{
	...
	public class MRRecommendationFullTextOnPage
	{
	...
	public class MRRecommendationFullText
	{
		private SQLCon _sQLCon;

		public MRRecommendationFullText(IAppSetting appSetting)
		{
			_sQLCon = new SQLCon(appSetting.GetConnectstring());
		}

		public MRRecommendationFullText()
		{
		}
		...
		public async Task<MRRecommendationFullText> MRRecommendationFullTextGetByID(long? Id)
		{
			DynamicParameters DP = new DynamicParameters();
			DP.Add("Id", Id);

			return (await _sQLCon.ExecuteListDapperAsync<MRRecommendationFullText>("MR_Recommendation_FullTextGetByID", DP)).ToList().FirstOrDefault();
		}

		public async Task<List<MRRecommendationFullText>> MRRecommendationFullTextGetAll()
		{
			DynamicParameters DP = new DynamicParameters();

			return (await _sQLCon.ExecuteListDapperAsync<MRRecommendationFullText>("MR_Recommendation_FullTextGetAll", DP)).ToList();
		}

		public async Task<List<MRRecommendationFullTextOnPage>> MRRecommendationFullTextGetAllOnPage(int PageSize, int PageIndex)
		{
			DynamicParameters DP = new DynamicParameters();

			DP.Add("PageSize", PageSize);
			DP.Add("PageIndex", PageIndex);
			return (await _sQLCon.ExecuteListDapperAsync<MRRecommendationFullTextOnPage>("MR_Recommendation_FullTextGetAllOnPage", DP)).ToList();
		}

		public async Task<int?> MRRecommendationFullTextInsert(MRRecommendationFullText _mRRecommendationFullText)
		{
			DynamicParameters DP = new DynamicParameters();
			DP.Add("RecommendationId", _mRRecommendationFullText.RecommendationId);
			DP.Add("FileId", _mRRecommendationFullText.FileId);
			DP.Add("FullText", _mRRecommendationFullText.FullText);

			return (await _sQLCon.ExecuteNonQueryDapperAsync("MR_Recommendation_FullTextInsert", DP));
		}

		public async Task<int> MRRecommendationFullTextUpdate(MRRecommendationFullText _mRRecommendationFullText)
		{
			DynamicParameters DP = new DynamicParameters();
			DP.Add("Id", _mRRecommendationFullText.Id);
			DP.Add("RecommendationId", _mRRecommendationFullText.RecommendationId);
			DP.Add("FileId", _mRRecommendationFullText.FileId);
			DP.Add("FullText", _mRRecommendationFullText.FullText);

			return (await _sQLCon.ExecuteNonQueryDapperAsync("MR_Recommendation_FullTextUpdate", DP));
		}

		public async Task<int> MRRecommendationFullTextDelete(MRRecommendationFullText _mRRecommendationFullText)
		{
			DynamicParameters DP = new DynamicParameters();
			DP.Add("Id", _mRRecommendationFullText.Id);

			return (await _sQLCon.ExecuteNonQueryDapperAsync("MR_Recommendation_FullTextDelete", DP));
		}

		public async Task<int> MRRecommendationFullTextDeleteAll()
		{
			DynamicParameters DP = new DynamicParameters();

			return (await _sQLCon.ExecuteNonQueryDapperAsync("MR_Recommendation_FullTextDeleteAll", DP));
		}
	}

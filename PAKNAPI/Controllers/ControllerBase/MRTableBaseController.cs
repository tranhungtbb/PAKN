...
namespace PAKNAPI.ControllerBase
{
	...
	public class MRTableBaseController : BaseApiController
	{
		...
		#region HISReommendation

		[HttpGet]
		[Authorize]
		[Route("HISReommendationGetByID")]
		public async Task<ActionResult<object>> HISReommendationGetByID(int? Id)
		{
			try
			{
				return new ResultApi { Success = ResultCode.OK, Result = await new HISReommendation(_appSetting).HISReommendationGetByID(Id) };
			}
			catch (Exception ex)
			{
		...
		[HttpGet]
		[Authorize]
		[Route("HISReommendationGetAll")]
		public async Task<ActionResult<object>> HISReommendationGetAll()
		{
			try
			{
				return new ResultApi { Success = ResultCode.OK, Result = await new HISReommendation(_appSetting).HISReommendationGetAll() };
			}
			catch (Exception ex)
			{
				new LogHelper(_appSetting).ProcessInsertLogAsync(HttpContext, ex);
		...
		[HttpGet]
		[Authorize]
		[Route("HISReommendationGetAllOnPage")]
		public async Task<ActionResult<object>> HISReommendationGetAllOnPage(int PageSize, int PageIndex)
		{
			try
			{
				List<HISReommendationOnPage> rsHISReommendationOnPage = await new HISReommendation(_appSetting).HISReommendationGetAllOnPage(PageSize, PageIndex);
				IDictionary<string, object> json = new Dictionary<string, object>
					{
						{"HISReommendation", rsHISReommendationOnPage},
						{"TotalCount", rsHISReommendationOnPage != null && rsHISReommendationOnPage.Count > 0 ? rsHISReommendationOnPage[0].RowNumber : 0},
						{"PageIndex", rsHISReommendationOnPage != null && rsHISReommendationOnPage.Count > 0 ? PageIndex : 0},
						{"PageSize", rsHISReommendationOnPage != null && rsHISReommendationOnPage.Count > 0 ? PageSize : 0},
					};
		...
		[HttpPost]
		[Authorize]
		[Route("HISReommendationInsert")]
		public async Task<ActionResult<object>> HISReommendationInsert(HISReommendation _hISReommendation)
		{
			try
			{
				new LogHelper(_appSetting).ProcessInsertLogAsync(HttpContext, null);

				return new ResultApi { Success = ResultCode.OK, Result = await new HISReommendation(_appSetting).HISReommendationInsert(_hISReommendation) };
			}
			catch (Exception ex)
			{
				new LogHelper(_appSetting).ProcessInsertLogAsync(HttpContext, ex);
		...
		[HttpPost]
		[Authorize]
		[Route("HISReommendationListInsert")]
		public async Task<ActionResult<object>> HISReommendationListInsert(List<HISReommendation> _hISReommendations)
		{
			try
			{
				int count = 0;
				int errcount = 0;
				foreach (HISReommendation _hISReommendation in _hISReommendations)
				{
					int? result = await new HISReommendation(_appSetting).HISReommendationInsert(_hISReommendation);
					if (result != null)
					{
						count++;
					}
		...
		[HttpPost]
		[Authorize]
		[Route("HISReommendationUpdate")]
		public async Task<ActionResult<object>> HISReommendationUpdate(HISReommendation _hISReommendation)
		{
			try
			{
				int count = await new HISReommendation(_appSetting).HISReommendationUpdate(_hISReommendation);
				if (count > 0)
				{
		...
		[HttpPost]
		[Authorize]
		[Route("HISReommendationDelete")]
		public async Task<ActionResult<object>> HISReommendationDelete(HISReommendation _hISReommendation)
		{
			try
			{
				int count = await new HISReommendation(_appSetting).HISReommendationDelete(_hISReommendation);
				if (count > 0)
				{
					new LogHelper(_appSetting).ProcessInsertLogAsync(HttpContext, null);
		...
		[HttpPost]
		[Authorize]
		[Route("HISReommendationListDelete")]
		public async Task<ActionResult<object>> HISReommendationListDelete(List<HISReommendation> _hISReommendations)
		{
			try
			{
				int count = 0;
				...
				foreach (HISReommendation _hISReommendation in _hISReommendations)
				{
					var result = await new HISReommendation(_appSetting).HISReommendationDelete(_hISReommendation);
					if (result > 0)
					{
						count++;
					}
		...
		[HttpPost]
		[Authorize]
		[Route("HISReommendationDeleteAll")]
		public async Task<ActionResult<object>> HISReommendationDeleteAll()
		{
			try
			{
				int count = await new HISReommendation(_appSetting).HISReommendationDeleteAll();
				if (count > 0)
				{
					new LogHelper(_appSetting).ProcessInsertLogAsync(HttpContext, null);
